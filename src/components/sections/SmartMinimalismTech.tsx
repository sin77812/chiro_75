...
const SmartMinimalismTech = () => {
  ...
  useEffect(() => {
    const checkDevice = () => {
      setIsMobile(window.innerWidth <= 1024) // Tablet and mobile detection for layout changes
    }
  ...
  const handleNodeHover = (nodeId: string, isHovering: boolean) => {
    // Skip scramble effects on mobile
    if (isMobile) return
      ...
      >
        ...
        <div 
          className={`
            ...
            ${isChiro ? 'font-sans tracking-wider leading-tight text-center font-black' : 'font-mono text-xs'}
          `}
          style={{
            whiteSpace: isChiro ? 'pre-line' : 'normal',
            fontSize: isChiro ? (isMobile ? 'clamp(16px, 4vw, 28px)' : 'clamp(24px, 6vw, 80px)') : undefined
          }}
        >
          {displayText}
        </div>
  ...
  return (
    <section 
      ref={sectionRef}
      className={`relative overflow-hidden bg-gradient-to-br from-black via-[#0A0A0A] to-[#0E1111] ${isMobile ? 'min-h-screen py-8 px-4' : 'h-[90vh]'}`}
      style={isMobile ? {} : { padding: '8vh 6vw' }}
    >
      ...
      {isMobile ? (
        ...
        <div className="w-full grid grid-cols-2 gap-4">
          ...
          {/* Row 3: CHIRO spanning 2 columns */}
          <div className="col-span-2">
            <TechNode key="chiro" node={techNodes.find(n => n.id === 'chiro')!} />
          </div>
          
          {/* Row 4: Performance Monitor spanning 2 columns */}
          <div className="col-span-2">
            <div className="bg-black/40 border border-[#1DB954]/20 rounded-xl p-3 backdrop-blur-sm">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-white/70">평균 전환율</span>
                  <span className="text-[#1DB954] font-bold">↑ {Math.floor(projectData.reduce((acc, p) => acc + p.conversion, 0) / projectData.length)}%</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-white/70">평균 로딩속도</span>
                  <span className="text-white">0.8s</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-white/70">가동률</span>
