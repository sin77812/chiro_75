...
const SmartMinimalismHero = () => {
  ...
  const calculateMagneticOffset = (elementRect: DOMRect, mouseX: number, mouseY: number) => {
    ...
    const maxDistance = 300  // Increased from 150 for wider effect range
    const maxOffset = 40      // Increased from 10 for more noticeable movement
  ...
  const MagneticChar = ({ char, index }: { char: string; index: number }) => {
    ...
    return (
      <span
        ...
        style={{
          transform: `perspective(1000px) translateX(${offset.x}px) translateY(${offset.y}px) translateZ(${Math.abs(offset.x) * 0.5}px) rotateY(${offset.x * 0.5}deg) rotateX(${-offset.y * 0.5}deg)`,
          transformStyle: 'preserve-3d',
          color: Math.abs(offset.x) > 10 || Math.abs(offset.y) > 10 ? '#1DB954' : 'white',
          textShadow: Math.abs(offset.x) > 10 || Math.abs(offset.y) > 10 ? '0 0 20px rgba(29, 185, 84, 0.5)' : 'none'
        }}
      >
        {char === ' ' ? '\u00A0' : char}
      </span>
